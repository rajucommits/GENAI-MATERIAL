# Streamlit application for database schema management and query processing
...
def initialize_session_state():
    ...
    defaults = {
        ...
        'show_execution_options': False,
        'active_tab': "Schema Generator",
        'db_config': {
        ...
        'domain': list(CONFIG["domains"].keys())[0] if CONFIG["domains"] else ""
    }
...
def format_audit_changes(changes: dict) -> str:
    """
    Format audit log changes for display.

    Args:
        changes (dict): Dictionary containing change details
...
def fetch_audit_logs(db_config: dict, page: int = 1):
    """
...
def fetch_schemas(db_config: dict, domain: str):
    """
    Fetch database schemas from the API.

    Args:
        db_config (dict): Database configuration
        domain (str): Domain to fetch schemas for
    """
...
def generate_full_description(db_config: dict, schema_name: str, table_name: str, domain: str):
    """
    Generate full table and column descriptions using the API.

    Args:
        db_config (dict): Database configuration
        schema_name (str): Schema name
        table_name (str): Table name
        domain (str): Domain for description generation
    """
    tab = "Schema Generator"
    st.session_state.messages[tab]["error"] = None
    st.session_state.messages[tab]["success"] = None
    ...
    with st.spinner(f"Generating descriptions for {schema_name}.{table_name}..."):
        try:
            ...
            ] + ([data] if not any(s["table_name"] == f"{schema_name}.{table_name}" for s in st.session_state.schemas) else [])
...
def update_descriptions(schema_name: str, table_name: str, db_config: dict, table_description: str, column_descriptions: dict):
    """
    Update table and column descriptions in PostgreSQL via the API.
    ...
    Args:
        ...
        column_descriptions (dict): Dictionary of column descriptions
    """
    tab = "Schema Generator"
    st.session_state.messages[tab]["error"] = None
    st.session_state.messages[tab]["success"] = None
...
def process_query(db_config: dict, query_input: str, domain: str):
    """
    Process a user query by sending it to the API for analysis.

    Args:
        db_config (dict): Database configuration
        query_input (str): User's query
        domain (str): Domain for query processing
    """
    tab = "Ask a Query"
    st.session_state.messages[tab]["error"] = None
    st.session_state.messages[tab]["success"] = None
...
def execute_queries(db_config: dict, execution_option: str, query_input: str):
    """
    Execute queries based on user-selected execution option.

    Args:
        db_config (dict): Database configuration
        execution_option (str): Selected execution option
        query_input (str): Original user query
    """
    tab = "Ask a Query"
    st.session_state.messages[tab]["error"] = None
    st.session_state.messages[tab]["success"] = None
        ...
        try:
            if execution_option == "Execute final query and summarize":
                ...
                summary_response = requests.post(
                    ...
                    json={"results": query_result["results"], "user_question": query_input, "rephrased_question": st.session_state.query_results["rephrased_question"]},
                    headers={"Content-Type": "application/json"}
                )
...
def main():
    ...
    with schema_tab:
        ...
        with col1:
            dbname = st.text_input("Database Name", value=st.session_state.db_config["dbname"])
        ...
        with col2:
            ...
            port = st.text_input("Port", value=st.session_state.db_config["port"])
            domain = st.selectbox("Domain", list(CONFIG["domains"].keys()),
                                  index=list(CONFIG["domains"].keys()).index(st.session_state.domain) if st.session_state.domain in CONFIG["domains"] else 0)
        ...
        # Generate table description section
        st.header("Generate Table Description")
        table_options = [table for table in CONFIG["domains"].get(domain, {}).get("tables", [])]
        selected_table = st.selectbox("Select Table to Generate Description", [""] + table_options, key="generate_table_select")
        if selected_table and st.button("Generate Description for Selected Table", key="generate_selected", type="primary"):
            schema_name, table_name = selected_table.split('.')
            generate_full_description(db_config, schema_name, table_name, domain)
            st.rerun()

        # Display and manage schemas
            ...
            if st.session_state.edit_table == schema["table_name"]:
                with st.form(key=f"edit_form_{schema['table_name']}"):
                    ...
                    table_description = st.text_area(
                        "Table Description",
                        value=st.session_state.edit_descriptions.get("table_description", schema["table_description"] or ""),
                    ...
                    for col in schema["columns"]:
                        col_desc = st.text_area(
                            ...
                            value=st.session_state.edit_descriptions.get("columns", {}).get(col["column_name"], col["description"] or ""),
                    ...
                    with col_save:
                        if st.form_submit_button("Save", type="primary"):
                            update_descriptions(schema_name, table_name, db_config, table_description, column_descriptions)
                            st.rerun()
                    with col_cancel:
                ...
                with col_update:
                    if st.button("Update Descriptions", key=f"update_{schema['table_name']}", type="primary"):
                        ...
                        update_descriptions(schema_name, table_name, db_config, schema["table_description"] or "", column_descriptions)
                        st.rerun()
                with col_generate:
                    if not schema["table_description"] and st.button("Generate Description", key=f"generate_{schema['table_name']}", type="primary"):
                        generate_full_description(db_config, schema_name, table_name, domain)
                        st.rerun()
    ...
    with query_tab:
        ...
        with col_submit:
            if st.button("Submit Query", key="submit_query", type="primary"):
                if not query_input.strip():
                    ...
                    process_query(st.session_state.db_config, query_input, st.session_state.domain)
                    st.rerun()
        ...
        if st.session_state.query_processed and st.session_state.query_results.get("rephrased_question"):
            ...
            # Extracted Tables
            if st.session_state.query_results.get("tables"):
                st.write("**Extracted Tables:**")
                st.write(f"{', '.join(st.session_state.query_results['tables'])}")

            # Extracted Columns
            if st.session_state.query_results.get("table_info"):
                st.write("**Table Information:**")
                for table_info_entry in st.session_state.query_results["table_info"]:
                    st.write(f"**{table_info_entry['table_name']}**: {table_info_entry.get('table_description', 'No description')}")
                    for col in table_info_entry.get("columns", []):
                        st.write(f"- {col['column_name']} ({col['data_type']}): {col.get('description', 'No description')}")

            # Sub-questions and queries
            if st.session_state.query_results.get("sub_questions") and st.session_state.query_results.get("sub_queries"):
                st.write("**Decomposed Sub-Questions and Queries:**")
                for sub_question, sub_query in zip(st.session_state.query_results["sub_questions"], st.session_state.query_results["sub_queries"]):
                    st.write(f"**Sub-Question:** {sub_question}")
                    st.code(sub_query, language="sql")

            # Final Query
            if st.session_state.query_results.get("final_query"):
                st.write("**Final Query:**")
                st.code(st.session_state.query_results["final_query"], language="sql")

        # Debug outputs
        if st.session_state.query_results.get("debug_final_prompt"):
            with st.expander("Debug: Final Summary Prompt"):
                st.write(st.session_state.query_results["debug_final_prompt"])

        if st.session_state.query_results.get("sub_query_results"):
        ...
        if st.session_state.show_execution_options and st.session_state.query_processed:
            ...
            if st.button("Execute Selected Option", key="execute_selected", type="primary"):
                execute_queries(st.session_state.db_config, execution_option, query_input)
                st.rerun()

        # Display execution results
        if st.session_state.query_results.get("sub_query_results") and len(st.session_state.query_results["sub_query_results"]) > 0:
            ...
            if st.session_state.query_results.get("summary"):
                st.subheader("Final Summary")
                st.write(st.session_state.query_results["summary"])

        elif st.session_state.query_results.get("results") is not None and len(st.session_state.query_results["results"]) > 0:
    ...
    with audit_tab:
        ...
        if st.button("Refresh Audit Logs", key="refresh_audit", type="primary"):
            fetch_audit_logs(st.session_state.db_config, page=1)
            st.rerun()
        ...
        if st.session_state.audit_logs:
            ...
            with col_prev:
                if st.button("Previous", key="audit_prev", type="primary", disabled=st.session_state.audit_page <= 1):
                    fetch_audit_logs(st.session_state.db_config, page=st.session_state.audit_page - 1)
                    st.rerun()
            with col_page_info:
