...
# AWS S3 and Secrets Manager configuration
...
def load_s3_file(bucket: str, key: str, local_path: str) -> str:
    try:
        s3_client.download_file(bucket, key, local_path)
        with open(local_path, "r") as f:
            return f.read()
    except botocore.exceptions.ClientError as e:
        logger.error(f"Error downloading {key} from S3 bucket {bucket}: {str(e)}")
        raise Exception(f"Failed to download {key} from S3: {str(e)}")
    except Exception as e:
        logger.error(f"Unexpected error loading {key} from S3: {str(e)}")
        raise

# Load configuration files from S3
CONFIG_PATH = "/tmp/config.json"
TABLE_CONTEXT_PATH = "/tmp/table.txt"

logger.info(f"Loading configuration from S3 bucket {S3_BUCKET_NAME}")
try:
    config_content = load_s3_file(S3_BUCKET_NAME, S3_CONFIG_KEY, CONFIG_PATH)
...
logger.info(f"Loading table context from S3 bucket {S3_BUCKET_NAME}")
try:
    table_content = load_s3_file(S3_BUCKET_NAME, S3_TABLE_KEY, TABLE_CONTEXT_PATH)
...
try:
    db_secrets = get_db_credentials(SECRETS_ARN)
    db_config = DBConfig(
        dbname=db_secrets.get("dbname", "olist_ecommerce"),
    ...
    logger.error(f"Error setting up DB configuration: {str(e)}")
    raise
